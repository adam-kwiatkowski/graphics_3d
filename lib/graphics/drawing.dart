...
import 'package:graphics_3d/graphics/utils.dart';
...
class Drawing extends ChangeNotifier {
  Drawing(this._size) {
    backgroundPixels = Uint8List(_size.width.toInt() * _size.height.toInt() * 4);
  }
  ...
  set size(ui.Size value) {
    _size = value;
    backgroundPixels = Uint8List(_size.width.toInt() * _size.height.toInt() * 4);
    for (var i = 0; i < backgroundPixels.length; i += 4) {
      backgroundPixels[i] = _canvasColor.red;
      backgroundPixels[i + 1] = _canvasColor.green;
      backgroundPixels[i + 2] = _canvasColor.blue;
      backgroundPixels[i + 3] = _canvasColor.alpha;
    }
  }

  final List<Shape> _objects = [];
  late Uint8List backgroundPixels;
  ...
  void floodFill(ui.Offset offset, ui.Color color) {
    final pixels = toBytes();
    final queue = Queue<ui.Offset>();
    final visited = <ui.Offset>{};
    final startColor = getPixel(offset, pixels, size);
    ...
    while (queue.isNotEmpty) {
      ...
      final neighbors = getNeighbors(current, size);
      for (var neighbor in neighbors) {
        if (visited.contains(neighbor)) continue;
        visited.add(neighbor);
        if (getPixel(neighbor, pixels, size) == startColor) {
          queue.add(neighbor);
        }
      }
      setPixel(current, pixels, size, color);
    }
  ...
  Uint8List toBytes() {
    ...
    // }
    final pixels = Uint8List.fromList(backgroundPixels);
